...
export async function formatTextfields(context, { keys, textfields }) {
    ...
    if (textfields.textarea && textfields.textarea.label)
        textfields.textarea.model = keys.observations
        ...
        for (let prop in textfields) {
            if (Array.isArray(textfields[prop])) {
                for (let i = 0; i < textfields[prop].length; i++) {
                    for (let key in textfields[prop][i]) {
                        if (k == textfields[prop][i][key]) {
                            if (textfields[prop][i].label === 'Estatus') {
                                textfields[prop][i] = getColorStatus(textfields[prop][i], keys[k])
                            } else if ((textfields[i] && textfields[i][prop] && textfields[i][prop].type === 'select')) {
                                textfields[prop][i] = getModelSelected(textfields[prop][i], keys[k])
...
function getColorStatus(item, status) {
    ...
    if (item.options) {
        if (status) {
            item.model = item.options[0]
        } else {
            item.model = item.options[1]
        }
    }
